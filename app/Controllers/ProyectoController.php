...
class ProyectoController extends Controller
{
  ...
  {
     $result = $this->model->getLimit('IdProyecto',0,20);
...
//y ejecuta las funciones que se encuentran en el archivo model de partidas
  public function save($request){
    if($this->verify($request)){
       header('location: '.FOLDER_PATH.'/proyecto');
    }else{
      if($request["id"] == 0){
  ...
  public function requested(){
    ...
    $Id = ''; $codigo = ''; $concepto = '';
    $result = $this->model->getById('IdProyecto',$_POST['id']);
    ...
    require_once ROOT . FOLDER_PATH .'/app/Request/ProyectoRequest.php';
  }
...
//Metodo para eliminar es llamado desde el archivo request.js
  ...
  public function search(){
    ...
    $result = $this->model->getAll('IdProyecto');
    if (!empty($_POST['date'])) {
    ...
    $funcion = $_POST['function'];
    $result = $this->model->getLimit('IdProyecto',($_POST['page']),($_POST['fin']));
    require_once ROOT . FOLDER_PATH .'/app/Request/ProyectoRequest.php';
  }
