...
?>
<form action = "<?php echo FOLDER_PATH ?>/Proyecto/save" method = "post" onsubmit="return validar();">
  <input type="hidden" name="id" value="<?= $Id; ?>"> <!-- es un campo invisible que sirve para mandar el id-->
  <div class="form-group row">
    <div class="col-sm-12">
      <label class="col-form-label">Codigo:</label>
          <input type="number" name="codigo" class="form-control" id="codigo" min="1" value="<?= $codigo ?>" autocomplete="off">
    </div>
 </div>
 <div class="form-group row">
   <label class="col-sm-12 col-form-label">Concepto:</label>
   <div class="col-sm-12">
     <textarea class="col" name="concepto" id="concepto" rows="4" onkeyup="mayus(this);"><?= $concepto ?></textarea>
   </div>
 </div>
...
?>
<form action = "<?php echo FOLDER_PATH ?>/Proyecto/remove" method = "post">
  <input type="hidden" name="Id" value="<?= $Id; ?>">
  <div class="form-group row">
    <div class="col-sm-12">
      <label class="col-form-label fa fa-exclamation-triangle"> ¿Seguro que desa eliminar el registro con codigo: <?= $codigo ?> ?</label>
    </div>
 </div>
 <div class="clearfix"></div><hr>
 <div class="form-group">
...
if ($_POST['method'] == 'pagination') {
 ...
 <tr>
  ...
  <td>
    <center>
    <button type="button" class="btn btn btn-outline-info btn-sm badge badge-pill" data-toggle="modal" data-target="#ventana" data-id="<?= $r["IdProyecto"] ?>" data-model="Partidas" data-operation="Editar" data-method="requested">
      <span class="icon-pencil"></span>
    </button>
 </center>
  </td>
  <td>
    <center>
    <button type="button" class="btn btn btn-outline-danger btn-sm badge badge-pill" data-toggle="modal" data-target="#ventana" data-id="<?= $r["IdProyecto"] ?>" data-model="Partidas" data-operation="Eliminar" data-method="delete">
      <span class="icon-trash"></span>
    </button>
 <td>
    <center>
