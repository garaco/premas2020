...
<html lang="es" dir="ltr">
  <head>
        <?php $header = new header();
          $header->PushHeader('Departamento');
     ?>
  </head>
      ...
      <nav class="navbar navbar-dark colornavbar">
        <div class="container-fluid">
          <div class="navbar-header">
            ...
            <a class="navbar-brand font-weight-bold text-white" >DEPARTAMENTOS</a>
          </div>
          <div class="col-sm-6" >
              <label class="sr-only">Buscar</label>
              <input type="text" class="form-control form-control-sm icon-heart-empty" placeholder="Buscar" name="buscar" id="buscar" data-model="Departamento" autocomplete="off">
            </div>
       ...
       <div class="row justify-content-md-center">
          <div class="col-sm-12 alert-color">
            <img src="<?php echo PATH_PUBLIC ?>/img/add.png">
              <label class="font-weight-bold">ALTA DE DEPARTAMENTO</label>
          </div>
        </div>
            <div class="row" >
            <!-- Button que manda a llamar la ventana modal -->
            <div class="form-group col-sm-1.5 p-3" >
                <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#ventana" style="float:right;" data-id="0" data-model="Departamento" data-operation="Agregar" data-method="requested">
        ...
        <div class="row">
          <div class="col-sm-12">
            <label class=" form-control text-center font-weight-bold" style="border: white;">
              <img src="<?php echo PATH_PUBLIC ?>/img/lista.png">LISTA DE DEPARTAMENTOS</label>
          </div>
        </div>
            <div class="input-group input-group-sm" >
              <div class="input-group-prepend ">
              <span class="input-group-text bg-white">Filtrar por </span>
              </div>
            <select class="badge" name="id" id="depa" data-model="Departamento">
              <option  value="0">Seleccione área</option>
                 <?php
              $area=new AreaModel();
              $area=$area->getall('IdArea');
              foreach ($area as $a) {?>
              <option value="<?=$a['IdArea']?>"> <?=utf8_encode($a['NombreArea'])?> </option>
              <?php
                }
              ?>
            </select>
            ...
            <div class="table table-responsive-sm table-responsive-md small">
              <table class="table table-sm table-striped table-hover" id="table_id1">
                ...
                <tbody class="container-table">
                  <?php foreach ($result as $r){ static $count = 1;?>
                  <tr>
