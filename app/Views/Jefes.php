...
<html lang="es" dir="ltr">
  <head>
     <?php
         $header = new header();
         $header->PushHeader('Jefes de Area');
     ?>
  </head>
      ...
      <nav class="navbar navbar-dark colornavbar">
        <div class="container-fluid">
          <div class="navbar-header">
            ...
            <a class="navbar-brand font-weight-bold text-white" >JEFES DE ÁREA</a>
          </div>
          <div class="col-sm-6" >
              <label class="sr-only">Buscar</label>
              <input type="text" class="form-control form-control-sm" placeholder="Buscar" name="buscar" id="buscar" data-model="Jefes" autocomplete="off">
          </div>
          <div class="dropdown ">
        ...
        <div class="row justify-content-md-center">
          <div class="col-sm-12 alert-color">
            <img src="<?php echo PATH_PUBLIC ?>/img/add.png">
              <label class="font-weight-bold">ALTA JEFES DE ÁREA</label>
          </div>
        </div>
          <div class="row">
            <div class="form-group col-sm-1.5 p-3" >
                <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#ventana" style="float:right" data-id="0" data-model="Jefes" data-operation="Agregar" data-method="requested">
        ...
        <div class="row">
          <div class="col-sm-12">
            <label class=" form-control text-center font-weight-bold" style="border: white;">
              <img src="<?php echo PATH_PUBLIC ?>/img/lista.png">LISTA DE JEFES DE ÁREAS</label>
          </div>
        </div>
            ...
            <div class="table table-responsive-sm table-responsive-md small">
              <table class="table table-sm table-striped table-hover" id="table_id1">
                ...
                <tbody class="container-table">
                  <?php foreach ($result as $j){static $count = 1;?>

                  <tr>
                    <th scope="row"><?= $count++;?></th>
                    <td><?=$j['Subfijo'].''.$j['Nombre'];?></td>
                    <td><?=$j['A_paterno'];?></td>
                    <td><?=$j['A_materno'];?></td>
                    <td><?=$j['Departamento'];?></td>
                    <td>
                      <center>
                        <button type="button" class="btn btn btn-outline-info btn-sm badge badge-pill" data-toggle="modal" data-target="#ventana" data-id="<?= $j["IdJefe"] ?>" data-model="Jefes" data-operation="Editar" data-method="requested">
                    ...
                    <td>
                      <center>
                        <button type="button" class="btn btn btn-outline-danger btn-sm badge badge-pill" data-toggle="modal" data-target="#ventana" data-id="<?= $j["IdJefe"] ?>" data-model="Jefes" data-operation="Eliminar" data-method="delete">
